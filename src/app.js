...
function getDomForCommand(command) {
  ...
  if (commandObj === undefined) {
    html = `'${
      command.split(" ")[0]
    }'is not recognized as an internal or external command, executable program, or batch file. Type <code>help</code> to display the list of internal commands.`;
  } else {
...
function handleCustomCommands(command) {
  switch (command) {
    ...
    case "get_cv":
      getCV();
      return "CV is coming ... wait for it ...";
    case "rm -rf /":
      rmRf();
